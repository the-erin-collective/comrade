...
function registerErrorHandlingCommands(context: vscode.ExtensionContext) {
    ...
    const showErrorRecoveryCommand = vscode.commands.registerCommand('comrade.showErrorRecovery', async (error: unknown) => {
        const actions = ['Retry', 'Configure', 'Report Issue'];
        const selectedAction = await vscode.window.showErrorMessage(
            `Comrade Error: ${error instanceof Error ? error.message : String(error)}`,
            ...actions
        );
        switch (selectedAction) {
            ...
            case 'Configure':
                ...
                if (typeof error === 'object' && error !== null && 'code' in error && typeof (error as any).code === 'string') {
                    ...
                    } else if ((error as any).code.includes('MCP')) {
